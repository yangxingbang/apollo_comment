...
// 只有on_lane_planning和navi_planning会调用traffic_decider.h
// 也就是说，交通决策就在这两代码中进行
...
Status OnLanePlanning::InitFrame(const uint32_t sequence_num,
  ...
  // 这个get函数背后是create和update函数，get到一条经过平滑的参考线
  // 平滑有三种方法，默认是离散点平滑，离散点平滑又有两种方法，默认使用FemPos
  // FemPos要求解二次规划，又有三种方法，默认使用不考虑曲率约束的osqp
  ...
  // 获取向前的距离，一般是8秒，最短180m，最长250m
  auto forward_limit =
      hdmap::PncMap::LookForwardDistance(vehicle_state.linear_velocity());

  // look_backward_distance向后的距离50m
  for (auto& ref_line : reference_lines) {
    // Vec2d把x，y两个值构成{x,y}这样的矢量
    // 把参考线分段
  ...
  // ego_info是一个类的对象
  // 私有成员变量有： 轨迹起始点，自车状态，前方空白区域的距离，自车车辆配置文件，车辆四边形的box
  // 轨迹的起始点在这里！！！
...
void OnLanePlanning::RunOnce(const LocalView& local_view,
  ...
  // local_view_是个结构体的对象，该对象访问了它的成员localization_estimate，
  // 成员是一个指针，在这个指针前边加*，返回了指针所指地址的对象
  // 这个对象是一个proto的msg，而形参是个string，为什么是合法的？

  // 用定位和底盘数据给vehicle_state喂数据
  ...
  // IsVehicleStateValid判断变量是否是NAN(not a number)
  ...
  // 假如当前路由和上时间步的路由不同，那么设入新路由，规划器初始化
  // 也就是说，当前路由要是和上个时间步的路由相同，那么继承上一步规划结果
  ...
  // 规划模块的运行被预测数据触发
  // perception100ms计算一次，所以100ms释放一次障碍物信息
  // prediction100ms收到一次障碍物信息，再加上自己的计算周期，应在100ms左右
  // 所以planning的计算时间在100ms左右，有可能超过100ms

  // 但是为了做轨迹拼接，这里设定planning_cycle_time为0.1s，即100ms
  const double planning_cycle_time =
      1.0 / static_cast<double>(FLAGS_planning_loop_rate);

  std::string replan_reason;
  // trajectory_stitching_preserved_length默认值20个点
  // 在极端情况下，轨迹拼接有没有可能导致轨迹点的个数变为0？
  ...
  // ego_info是一个类的对象，这个对象包括车辆状态，车辆的box，轨迹的起始点等
  injector_->ego_info()->Update(stitching_trajectory.back(), vehicle_state);
  // 帧的序号
  const uint32_t frame_num = static_cast<uint32_t>(seq_num_++);

  // 获取（经过平滑的）参考线
  // 用参考线，当前时刻轨迹起始点，车辆状态等去初始化一个帧frame
  ...
  // apollo里的replan是更换起始点，等下次消息触发planning_component时再规划
...
Status OnLanePlanning::Plan(
  ...
  // 选用规划器，除过rtk_planner以外，规划器有3种:lattice，navi，public_road
  ...
  if (frame_->open_space_info().is_on_open_space_trajectory()) {
    ...
    // 代价最小的参考线
    // 之前是给每个参考线的多条path求cost最小
    // 这个参考线对应的最小cost的path不一定是所有参考线对应的所有path中cost最小的那一条
    const auto* best_ref_info = frame_->FindDriveReferenceLineInfo();
    // 多余的吗？其实还是best_ref_info
