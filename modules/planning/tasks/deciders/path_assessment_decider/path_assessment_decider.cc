...
// tuple,元组，是泛化的pair；不能遍历，取值用get<常量>方法
...
Status PathAssessmentDecider::Process(
  ...
  // path reuse decider尚未看
  if (FLAGS_enable_skip_path_tasks && reference_line_info->path_reusable()) {
    return Status::OK();
  }

  // 在jerk_path_optimizer中设定候选路径，在这里获取候选路径
  // candidate_path_data里存了两条路径及其边界
  ...
  // curr_path_data表示一条路径，是一系列点
  // 每条路径会打一个lable，参见path boundary decider.cc
  // 所以标签有且只会有fallback和regular
...
void PathAssessmentDecider::SetPathInfo(
  ...
  // PathPointDecision是一个tuple元组，里边有三个元素
  // 第一个元素是点的s值，第二个元素是点的类型，从后文get<2>可知第三个是自车到障碍物的距离
  // 这里的障碍物类型一定是静止障碍物吗？注意需要区分障碍物是不是被忽略的
  // 点的类型包括in lane，out of lane， off road等
  ...
  if (reference_line_info.IsChangeLanePath()) {
    ...
    // 标记出换道点的位置?确定自车与车道的位置关系，是否in lane
...
bool PathAssessmentDecider::IsGreatlyOffRoad(
  ...
  for (const auto& frenet_path_point : frenet_path) {
    ...
    // 给车道横向划分格子，然后计算车辆距离左右道路边界的距离，再与阈值比较
    ...
    if (reference_line_info.reference_line().GetLaneWidth(
      ...
      if (is_lane_change_path) {
        ...
        if (ego_sl_boundary.start_l() > lane_left_width ||
          ...
          // 还没有开始换道的时候，box对应的boundary肯定是大于一个车道的
          // 假如相邻车道空闲，那boundary就有两个车道宽
          ...
          // 换道点应该越过车道后，再越过一个buffer，才算换到另一个车道
          // 在这个时候boundary是小于车道宽减去一个buffer的
          std::get<1>((*path_point_decision)[i]) =
              PathData::PathPointType::IN_LANE;
        } else {
        ...
        // on reverse lane!
        // 对于借道，一旦车辆不在当前参考线的车道，而在其他车道，甚至是在对向车道
        // 计算boundary时，借道与换道的buffer是不同的，而在此计算车辆处于车道位置时
        // 用的buffer是相同的！
        // TODO(yxb)： 车的一角出了车道，一角还没有出车道？我需要仔细理解这里的判断条件？
        if (ego_sl_boundary.end_l() >
          ...
          // 借道点肯定出现在车辆前方，对向车道或者自己车道，不会出现在车辆的尾部后方
          // 这是不符合逻辑的
