...
class PathBoundsDecider : public Decider {
 ...
 private:
  ...
  // 正常的路径边界生成会考虑到自车和其他静态环境的：
  // 自车位置，车道消息，静态障碍物，要被借道的车道，阻塞车道障碍物的id
  // 哲学思想是：路径规划必须考虑静态环境
   ...
   * @return common::Status
   */
  // 备用路径边界是最后一道保险。保证速度规划器有路径可用而不会失败。
  // 有了可信的备用路径边界，优化器不会失败。
  // 备用路径边界被启用时，速度规划器要考虑车辆会停在静态障碍物前边。同时也不考虑借道。
