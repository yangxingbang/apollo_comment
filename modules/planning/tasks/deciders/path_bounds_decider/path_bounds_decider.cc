...
// 我们也可以把他当做一个通用的结构体来用，不需要创建结构体又获取结构体的特征，
// 在某些情况下可以取代结构体使程序更简洁，直观。std::tuple理论上可以有无数个
// 任意类型的成员变量，而std::pair只能是2个成员，因此在需要保存3个及以上的
// 数据时就需要使用tuple元组了。

// tuple不支持迭代，只能通过元素索引(或tie解包)进行获取元素的值。
// 但是给定的索引必须是在编译器就已经给定，不能在运行期(比如for中的i)
// 进行动态传递，否则将发生编译错误
...
Status PathBoundsDecider::Process(
  ...
  // 如果有靠边停车，生成靠边停车的路径边界
  // 注意这里是在candidate_path_boundaries中塞完所有的fallback路径点边界，
  // 然后再塞pullover/lanechange/self路径的路径点边界
  // 所以这个数据结构中包含两种类型路径的边界
  ...
  // 在有了常规边界以后，再检验是否有借道，如果有，再考虑借道的边界生成
  for (const auto& lane_borrow_info : lane_borrow_info_list) {
    ...
    // log中的blocking_obstacle_id在这里生成
    // ADEBUG << "borrow_lane_type" << borrow_lane_type;
    if (!ret.ok()) {
      continue;
    }
...
void PathBoundsDecider::InitPathBoundsDecider(
  ...
  // ADC s/l info.
  // adc_sl_info是一个由两个数组组成的键值对pair
  // 把自车规划起始点从全局坐标系转换到frenet坐标系
  ...
  // 用自车规划起始点的s去找 原始参考线生成的平滑参考线上的最近点对应的 关键路点
  // 计算规划起始点l与关键路点的l的差值
...
Status PathBoundsDecider::GenerateFallbackPathBound(
  ...
  // 2. Decide a rough boundary based on lane info and ADC's position
  // fallback的路径边界只考虑自车信息和道路信息，不考虑静态障碍物？
  // 由速度决策器决定车辆停在障碍物之前？
  std::string dummy_borrow_lane_type;
  if (!GetBoundaryFromLanesAndADC(reference_line_info,
...
bool PathBoundsDecider::InitPathBoundary(
  ...
  // 实际边界是100和8s车速路长的最大值，再取该最大值和参考线长度的最小值
  for (double curr_s = adc_frenet_s_;
    ...
    // 依次塞入每个s位置的l向的最小值和最大值
    // 初始化时放的是双精度数的最小值-1.79769e+308与最大值1.79769e+308
...
bool PathBoundsDecider::GetBoundaryFromLanesAndADC(
  ...
  for (size_t i = 0; i < path_bound->size(); ++i) {
    ...
    if (!reference_line.GetLaneWidth(curr_s, &curr_lane_left_width,
      ...
      double curr_road_left_width = 0.0;
      double curr_road_right_width = 0.0;
      // boundary_buffer默认为0.05
      if (reference_line.GetRoadWidth(curr_s, &curr_road_left_width,
                                      &curr_road_right_width)) {
        // 如果路的边界比车道线的边界宽了0.05米以上，就取左边车道线
      ...
      // 获取当前s位置参考线上点距离中心线的l向距离
      reference_line.GetOffsetToMap(curr_s, &offset_to_lane_center);
      // 采用右手sl坐标系，车辆在中心线左边的时候，offset是正值
      // 因此curr_lane_left_width是变大了的
      // curr_lane_left_width是什么呢？怎么计算的呢？尚未懂
      curr_lane_left_width += offset_to_lane_center;
      // curr_lane_right_width是变小了的
      curr_lane_right_width -= offset_to_lane_center;
      past_lane_left_width = curr_lane_left_width;
      past_lane_right_width = curr_lane_right_width;
    }
    ...
    //    and ADC's velocity.
    // 用车道宽度，参考线上规划起始点的位置，参考线上规划起始点的速度来计算合适的边界
    // 计算 规划路径边界的时候，取的横向加速度的最大值是1.5，这会不会称为一个引起错误的地方，
    // 假如大横向加速度点的时候？
    static constexpr double kMaxLateralAccelerations = 1.5;
    double offset_to_map = 0.0;
    reference_line.GetOffsetToMap(curr_s, &offset_to_map);

    // 对横向来说，就是横向速度变为0的过程⾏驶的横向位移
    double ADC_speed_buffer = (adc_frenet_ld_ > 0 ? 1.0 : -1.0) *
                              adc_frenet_ld_ * adc_frenet_ld_ /
                              kMaxLateralAccelerations / 2.0;

    // 假如向左借道，边界就加上左边的相邻车道宽
    ...
    // is_extend_lane_bounds_to_include_adc什么意思？
    if (config_.path_bounds_decider_config()
            .is_extend_lane_bounds_to_include_adc() ||
      ...
      // bounds.
      // 在备用路线或者换道中可以扩大路径的边界
      // 此时靠考虑参考线与中心线的横向相对位置，车辆的横向速度，车宽的一半，
      double curr_left_bound_adc =
          std::fmax(adc_l_to_lane_center_,
...
std::vector<ObstacleEdge> PathBoundsDecider::SortObstaclesForSweepLine(
  ...
  for (const auto* obstacle : indexed_obstacles.Items()) {
    ...
    // 绕障时需要修改的障碍物参数obstacle_lat_buffer
