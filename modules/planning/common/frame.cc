...
Status Frame::Init(
  ...
  // 把障碍物，地图，预测，车辆状态再检查一遍，尤其检查自车与障碍物是否碰撞
  // 然后把帧里边需要的数据全部灌到frame里去
...
Status Frame::InitFrameData(
  ...
  // 默认为假
  // 为什么要在此处对齐车辆状态和预测障碍物的时间戳？
  // 一定是为了保证预测车和自车的状态是同时的，否则容易发生碰撞，或者匹配失效！
...
const Obstacle *Frame::FindCollisionObstacle(const EgoInfo *ego_info) const {
  ...
  // 获取当前车辆box的角点，然后用这些角点生成一个凸的多边形，是自车的多边形
...
// 注意，这里的perception是全小写的，它是个命名空间，不是一个类！
