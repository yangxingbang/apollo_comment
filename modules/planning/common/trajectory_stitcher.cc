...
TrajectoryStitcher::ComputeReinitStitchingTrajectory(
  ...
  if (std::abs(vehicle_state.linear_velocity()) < kEpsilon_v &&
    ...
    // 自行车运动学模型预测车辆的在规划周期内走的距离，得到那个位置作为reinit_point
...
// 假如自动驾驶模式关，或者上一个规划循环没有生成轨迹，或者实际到目标的位置距离太远
// 那么从当前位置开始规划
// 这就是重规划！
...
std::vector<TrajectoryPoint> TrajectoryStitcher::ComputeStitchingTrajectory(
  ...
  // 当前时间戳 减去 上条轨迹的时间戳 得到相对时间
  const double veh_rel_time =
      current_timestamp - prev_trajectory->header_time();

  // 比veh_rel_time短epsilon的时间长度内的轨迹点，epsilon极其小
  // 相当于就是在veh_rel_time时间长度内的轨迹点
  ...
  // forward_time_index对应点往“以前方向”退后最多20个点作为拼接轨迹的起始点
  // forward_time_index对应点的下一个点作为拼接轨迹的终点
  // 因此，拼接轨迹的点数是20个左右！
  ...
  // back()函数返回最后一个元素的地址
  // 所以取的是缝合轨迹的最后一个点作为起点
  // 既然只用到缝合轨迹的最后一个点作为起点，那还在缝合轨迹的起始位置做那么多限定有什么意义？
  // 只是为了减少点的个数来减少资源消耗吗？
