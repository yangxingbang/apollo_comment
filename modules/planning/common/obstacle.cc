...
void Obstacle::AddLongitudinalDecision(const std::string& decider_tag,
  ...
  // 先merge一个无对象的longitudinal_decision_和一个有对象的decision
  // 以后merge一个存在历史对象的longitudinal_decision_和一个有对象的decision
...
void Obstacle::CheckLaneBlocking(const ReferenceLine& reference_line) {
  ...
  // 这样的s-l只能确定两个点，sl边界的宽度取的是车道的宽度，结合起来形成一个可行域
  ...
  // 判断sl边界是否在车道内
  // static_obstacle_nudge_l_buffer默认0.3m
