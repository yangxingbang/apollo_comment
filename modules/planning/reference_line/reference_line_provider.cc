...
bool ReferenceLineProvider::GetReferenceLines(
  ...
  if (FLAGS_enable_reference_line_provider_thread) {
    ...
    if (!reference_lines_.empty()) {
      // 将元素从现有列表复制到新列表
      // 把存在的参考线的元素从头到位复制到新参考线变量
      // 把路由片段的元素从头到位复制到新路由片段变量
      // route_segment和PnC地图中的road_segment不是同一个东西
  ...
  // reference_line和segment存在池子中
  // 在生成新reference_line的时候，把他们取出来用，生成以后再存进池子
...
bool ReferenceLineProvider::CreateRouteSegments(
    ...
    std::list<hdmap::RouteSegments> *segments) {
  {
    ...
    // TODO(yxb):add for route segment test
    for (auto it = segments->begin(); it != segments->end(); it++) {
      ADEBUG << "size of lane segments in each route segment"
             << (*it).size();
    }
  }
...
bool ReferenceLineProvider::CreateReferenceLine(
  ...
  bool is_new_routing = false;
  {
    ...
    // PnC_map是把高精地图和相对地图的元素解构成规划要用的元素，统称为PnC_map
    std::lock_guard<std::mutex> lock(pnc_map_mutex_);
...
AnchorPoint ReferenceLineProvider::GetAnchorPoint(
  ...
  AnchorPoint anchor;
  // longitudinal_boundary_bound默认值是2
  // max_lateral_boundary_bound默认值0.5
  // min_lateral_boundary_bound默认值0.1
  ...
  // 车道变弯后，能给车辆通行的横向车道宽变小了，因为车辆是个刚体，必与曲线的切线形成夹角
  // 而直道时，车体可与边界平行
  // 具体计算尚未明白！
  if (hdmap::RightBoundaryType(waypoint) == hdmap::LaneBoundaryType::CURB) {
    // 离散点平滑中curb_shift默认是0.2
...
void ReferenceLineProvider::GetAnchorPoints(
  ...
  // 离散点平滑中max_constraint_interval默认值0.25
