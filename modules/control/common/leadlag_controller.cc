...
double LeadlagController::Control(const double error, const double dt) {
  ...
  // 为什么这么定义内部状态？
  // 公式推导中的需要，所以定义了一个变量，目前还不清楚innerstate_是何物理意义
...
void LeadlagController::Init(const LeadlagConf &leadlag_conf, const double dt) {
  ...
  innerstate_ = 0.0;
  // 内部状态的上下限是依据什么定义的？
  // innerstate_saturation_level有什么真实的物理意义？
...
// 要注意到函数中的方程是Z变换后的形式
// 我推测这里将s域的传递函数变换到z域，再到计算output时变换回时域
// 只是因为z域的公式是加减乘除，更加适合代码表达
