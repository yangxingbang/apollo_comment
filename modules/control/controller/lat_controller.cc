...
LatController::LatController() : name_("LQR-based Lateral Controller") {
  if (FLAGS_enable_csv_debug) {
    ...
    // 表示浮点输出应该以固定点或小数点表示法显示，不会出现科学计数法显示
...
bool LatController::LoadControlConf(const ControlConf *control_conf) {
  ...
  // 四轮质量分布
  // 这里测量出每个轮子分配的整车质量并没有意义，因为单项都没有被用到
  ...
  // 相对于质心的转动惯量，用于横摆，会用于横向速度加速度力的计算吗？
  // moment of inertia
...
void LatController::InitializeFilters(const ControlConf *control_conf) {
  ...
  // 均值滤波
  // mean_filter_window_size 均值滤波使用数据的个数
  // 为什么有的用二阶低通滤波，有的用均值滤波，怎么选择？ 
...
Status LatController::ComputeControlCommand(
  ...
  // 使用导航模式，导航模式的相对地图是在车辆坐标系下的，影响定位数据？
  if (FLAGS_use_navigation_mode &&
    ...
    // 当有新路径数据进来的时候，就把车辆的初始位置重新定义在当前位置
    // 是说时间戳变化很小的时候，坐标系没变动，时间戳变化稍大的时候，坐标系变动吗？
    // 其实时间戳变化小于1.0e-6的代码分支无意义，因为不存在
    // 或许用来debug才是它存在的真实目的？
  ...
  // 只是把轨迹的表达从后轴中心移到质心
  // 只有激活预瞄的时候才会执行这个坐标转换，为什么预瞄非得以质心作为参考点，后轴中心不行吗？
  // 甚至前轴中心不行吗？
  ...
  */
  ...
  if (FLAGS_enable_gain_scheduler) {
    // 调度增益实际上是利用速度修正了横向误差和航向误差的Q矩阵中对应值
  ...
  // 利用超前滞后环节，实现 转向角反馈量的提升
  // 如果车速在低速限值以下的窗口内，还需要对“提升量”差值
  if (enable_leadlag_) {
  ...
  // 以最大转向角速度在delta_T时间内产生的角度变化量，转化为转向百分比
  ...
  if (enable_mrac_) {
    ...
    // 假如线速度大于分辨率，默认0.2，那么设置 “自适应率”
    // 表示在车辆有速度的时候，才会设置自适应率
    if (std::fabs(vehicle_state->linear_velocity()) >
  ...
  // executed
  // lock_steer_speed： 速度很低时，限值转向，就是限制原地打方向盘
...
// 形参中的x，y是质心的位置坐标,theta是车辆当前的航向角
...
void LatController::ComputeLateralErrors(
  ...
  if (FLAGS_query_time_nearest_point_only) {
    ...
    // enable_navigation_mode_position_update一直保持为真
    // use_navigation_mode一直保持为假
    // 使用导航模式，但不用导航模式去更新位置，表示什么意思？
    // 不用它更新位置，那还有什么用？
    // 不用它更新位置，会不会出现数据不配套？
    // 不使用导航模式更新位置时，采用搜索时间范围来找目标点的
    if (FLAGS_use_navigation_mode &&
        !FLAGS_enable_navigation_mode_position_update) {
      target_point = trajectory_analyzer.QueryNearestPointByAbsoluteTime(
          Clock::NowInSeconds() + query_relative_time_);
  ...
  // 这里为什么没有计算delta_theta？再求三角函数
  // 是因为这里当前位置和目标点位置在同一个坐标系下吗？
  ...
  // enable_navigation_mode_error_filter默认为假
  ...
  if (injector_->vehicle_state()->gear() == canbus::Chassis::GEAR_REVERSE) {
    ...
    // 前进挡时，预测了一个当前速度和当前航向走完前视距离的时刻，然后用那时刻找了最近点
...
void LatController::UpdateDrivingOrientation() {
  ...
  // reverse_heading_control变量一直保持为假，下边的代码不会执行
  if (FLAGS_reverse_heading_control) {
    if (vehicle_state->gear() == canbus::Chassis::GEAR_REVERSE) {
