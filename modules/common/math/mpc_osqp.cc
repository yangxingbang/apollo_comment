...
MpcOsqp::MpcOsqp(const Eigen::MatrixXd &matrix_a,
  ...
  // 涉及到的参数的个数
  // 10个预测时域，每个预测时域有6个状态量，2个控制量
  // 因为离散空间表达的时候，采用X(k+1) = A * X(k) + B * u(k)的形式，只是形式，和本文无关
  // 因此，当前时刻，就会有当前和下一时刻共两个时刻的状态量，而只有一个时刻的控制量
  // 所以，状态量的horizon+1，而控制量的不用horizon+1
  // 86
...
// 参考状态是状态变量的参考值,因为状态变量都是误差量，所以参考值都是0
// 所以在这个二次规划中，gradient_全是0
...
void MpcOsqp::CalculateEqualityConstraint(std::vector<c_float> *A_data,
  ...
  // block matrix
  // 6*(10+1)+6*(10+1)+2*10行，6*(10+1)+2*10列
  // 这里x等式约束有 state_dim_ * (horizon_ + 1) 行
  // 这里x不等式约束有 state_dim_ * (horizon_ + 1) 行
  // 这里u的不等式约束有 control_dim_ * horizon_ 行
...
bool MpcOsqp::Solve(std::vector<double> *control_cmd) {
  ...
  // vector的at与下标相比，会进行 下标越界检查 ， 但是会增加性能开销
